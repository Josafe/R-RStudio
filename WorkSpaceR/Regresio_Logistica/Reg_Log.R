...
datos = read.csv('/home/itibcn/WorkSpaceR/Regresio_Logistica/Titanic-Dataset.csv')
head(datos)
str(datos)
...
grafico = grafico + geom_boxplot(aes(group=Pclass, fill=factor(Pclass),alpha=5))
print(grafico)
...
datos = select(dato, -PassengeId, -Name, -Ticket, -Cabin)
head(datos)
...
modelo <- glm(Survivded ~ . , family=binomial(link='logit'), data=entrenamiento)
